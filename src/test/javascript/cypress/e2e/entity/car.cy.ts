...
describe('Car e2e test', () => {
  ...
  const carSample = { model: 'overriding hard Refined', year: 'leverage upward-trending Republic)', available: 57260 };
  ...
  describe('Car page', () => {
    ...
    describe('with existing value', () => {
      ...
      it.skip('edit button click should load edit Car page and save', () => {
        cy.get(entityEditButtonSelector).first().click();
        cy.getEntityCreateUpdateHeading('Car');
  ...
  describe('new Car page', () => {
    ...
    it('should create an instance of Car', () => {
      cy.get(`[data-cy="model"]`).type('whiteboard').should('have.value', 'whiteboard');

      cy.get(`[data-cy="year"]`).type('Rupee Grocery').should('have.value', 'Rupee Grocery');

      cy.get(`[data-cy="available"]`).type('61219').should('have.value', '61219');
